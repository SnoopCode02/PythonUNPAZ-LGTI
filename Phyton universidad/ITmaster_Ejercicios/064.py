...
CANTIDAD_DE_TANDAS = 2
mayor_kg = -float('inf')
total = 0
for comida in range(CANTIDAD_DE_TANDAS):
    print(f"Comida {comida + 1} de {CANTIDAD_DE_TANDAS}")
    terminar = False
    while not terminar:
        kg = int(input("kg: "))
        total += kg
...
print(f"Total: {total}")
print(f"Promedio por tanda: {total/CANTIDAD_DE_TANDAS}")
